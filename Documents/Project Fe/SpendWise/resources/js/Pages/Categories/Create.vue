...
function submit() { form.post(route('categories.store')); }
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold text-ktext">✨ Create Category</h1>
            <p class="text-sm text-gray-500">Add a new category to organize your transactions</p>
        </div>
            ...
            <div class="rounded-3xl bg-white p-8 shadow-sm shadow-kpink/10 ring-1 ring-kpink/10">
                <form @submit.prevent="submit" class="space-y-5">
                    <div>
                        ...
                        <input v-model="form.name" type="text" placeholder="e.g. Food, Transport, Shopping" class="mt-1.5 block w-full rounded-2xl border-2 border-kpink/20 bg-white px-5 py-3 text-sm shadow-sm transition placeholder:text-gray-300 focus:border-kpink/50 focus:ring-4 focus:ring-kpink/10 focus:outline-none" />
