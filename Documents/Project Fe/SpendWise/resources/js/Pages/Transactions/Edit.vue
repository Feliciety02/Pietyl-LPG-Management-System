...
const props = defineProps({ transaction: Object, categories: Array });
...
const form = useForm({
    category_id: props.transaction.category_id, title: props.transaction.title,
    amount: props.transaction.amount, type: props.transaction.type,
    transaction_date: props.transaction.transaction_date, notes: props.transaction.notes || '',
});
function submit() { form.put(route('transactions.update', props.transaction.id)); }
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold text-ktext">✏️ Edit Transaction</h1>
            <p class="text-sm text-gray-500">Update your transaction details</p>
        </div>
            ...
            <div class="rounded-3xl bg-white p-8 shadow-sm shadow-kpink/10 ring-1 ring-kpink/10">
                <form @submit.prevent="submit" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input v-model="form.title" type="text" class="mt-1.5 block w-full rounded-2xl border-2 border-kpink/20 bg-white px-5 py-3 text-sm shadow-sm transition placeholder:text-gray-300 focus:border-kpink/50 focus:ring-4 focus:ring-kpink/10 focus:outline-none" />
